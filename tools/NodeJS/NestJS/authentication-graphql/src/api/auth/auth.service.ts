...
export class AuthService {
  ...
  public async register(res: Response, registerInput: RegisterInput) {
  ...
  public async login(res: Response, loginInput: LoginInput) {
    const user = await this.userService.findByEmail(loginInput.email);
    if (!user) {
      throw new UnauthorizedException('Invalid credentials');
    }

    const passwordValid = await verify(user.password, loginInput.password);
    if (!passwordValid) {
      throw new UnauthorizedException('Invalid credentials');
    }
